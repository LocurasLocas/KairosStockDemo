...
def build_remito_pdf(venta, cfg) -> bytes:
    """
    Genera un PDF de remito de caja con el mismo estilo que los presupuestos.
    venta: instancia de Venta
    cfg: instancia de RemitoConfig
    """
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.styles import ParagraphStyle
    from reportlab.lib.units import cm
    ...
    tipo = cfg.tipo_comprobante or 'Remito'
    title_lines = [
        Paragraph(tipo.upper(), title_style),
        Paragraph(f'N° {venta.ticket_number}', sub_style),
    ]

    if company_lines:
        header_table = Table(
            [[company_lines, title_lines]],
            colWidths=[9 * cm, 8.3 * cm]
        )
        header_table.setStyle(TableStyle([
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ...
        story.append(Paragraph(tipo.upper(), title_style))
        story.append(Paragraph(f'N° {venta.ticket_number}', sub_style))
    ...
    metodo = venta.payment_method.capitalize() if venta.payment_method else '-'
    ...
    info_table = Table(info_data, colWidths=[12 * cm, 5.3 * cm])
    info_table.setStyle(TableStyle([
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 8),
        ('TEXTCOLOR', (0, 0), (-1, 0), brand),
    ...
    # ── Totales ──────────────────────────────────────────────────────────────
    totals_data = [['', 'Subtotal:', f'${venta.subtotal:,.2f}']]
    if venta.tax_rate > 0 and cfg.mostrar_iva:
        totals_data.append(['', f'IVA ({venta.tax_rate:.0f}%):', f'${venta.tax_amount:,.2f}'])
    totals_data.append(['', 'TOTAL:', f'${venta.total:,.2f}'])
    ...
    if venta.notes:
        ...
        story.append(Spacer(1, 0.3 * cm))
        story.append(Paragraph('<b>Notas:</b>', note_style))
        story.append(Paragraph(venta.notes, note_style))

    # ── Pie de página ────────────────────────────────────────────────────────
    pie = cfg.pie_texto or '¡Gracias por su compra!'
    story.append(Spacer(1, 0.8 * cm))
    story.append(HRFlowable(width='100%', thickness=0.5, color=colors.HexColor('#e2e8f0')))
    story.append(Spacer(1, 0.2 * cm))
    footer_style = ParagraphStyle('footer', fontSize=8, textColor=muted, alignment=1)
